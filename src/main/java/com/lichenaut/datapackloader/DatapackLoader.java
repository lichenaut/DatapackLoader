...
import com.lichenaut.datapackloader.commands.ImportCommand;
...
public final class DatapackLoader extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if(config.getBoolean("developer-mode")) {
            try {
                new DLWorldsDeleter(plugin).deleteWorlds(levelName);
            } catch (IOException e) {
        ...
        if (activeDatapacks != null) {
            ...
            for (File file : Objects.requireNonNull(datapacksFolder.listFiles())) {if (file.isDirectory()) {currentDatapacks.add(file.getName());}}
            ...
            for (String name : currentDatapacks) {activeDatapacks.put(name, "hand");}
        }
        ...
        try {
            for (String stringUrl : config.getStringList("datapack-urls")) {
                if (!stringUrl.endsWith(".zip")) {console.sendMessage(ChatColor.RED + "[DatapackLoader] URL '" + ChatColor.RESET + stringUrl + ChatColor.RED +
                        "' must end with a .zip file! Skipping.");continue;}
                URL url = new URL(stringUrl);
                if (importChecker.checkUnnecessaryImport(url)) {urlImporter.urlImport(url);}
            }
        ...
        if (activeDatapacks != null) {
            try (FileWriter fileWriter = new FileWriter(activeDatapacksTextFile)) {
                for (Map.Entry<String, String> entry : activeDatapacks.entrySet()) {fileWriter.write(entry.getKey() + " " + entry.getValue() + "\n");}
            } catch (IOException e) {
        ...
        if (importEvent) {getServer().shutdown();}
